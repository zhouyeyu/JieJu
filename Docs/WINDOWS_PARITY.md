...
- [x] `WIN-204` 独立词语解释、生词收藏、释义/来源合并、详情、删除与回到来源
- [ ] `WIN-205` 识别卡、四档熟悉程度、每轮十张温和暂停、只追加复习日志
- [x] `WIN-206` 自动区分词语/表达/句子/段落，保留手动切换，并为英文残缺词提供边界建议
...
`WIN-204` 使用独立 v4 词语请求和 Ollama 结构化结果，中文解释未使用中文时自动修复一次；保存时
按语言、原形和读音合并词条，并累积释义与来源。生词本提供列表、详情、确认删除和返回最近来源。

`WIN-206` 根据标点、词数、语言和连续文字长度保守分类选区，默认进入对应解释流程，同时保留
“按句子/按词语解释”的人工切换。英文唯一残缺词可建议完整边界；日语边界修正依赖 `WIN-305`
本地形态分析，在此之前不猜测扩展日语选区。
