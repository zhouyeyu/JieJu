...
- [x] `WIN-203` 保存解释、学习记录详情、确认删除与从 EPUB locator 回到原文
...
`WIN-203` 将学习资料以共享 `learning-library` v5 原子写入 `%LOCALAPPDATA%\JieJu\library.json`；
重复选区更新原记录，损坏文件先备份再恢复。学习记录提供列表、完整解释详情、确认删除和返回原书
章节/章内位置。设备绝对路径仍只保留在 Windows 最近阅读状态中。
