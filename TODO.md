...
- [ ] `EPUB-202` 建立受限的 EPUB 本地资源访问层，禁止访问书外文件和网络
- [ ] `EPUB-203` 用 `WKWebView` 渲染章节 XHTML，保留标题、强调、图片和原书基础 CSS
- [ ] `EPUB-204` 注入阅读器 CSS，按视口多栏分页而非“一章一页”
- [ ] `EPUB-205` 实现上一页/下一页、跨章衔接和键盘翻页
- [ ] `EPUB-206` 计算当前章节页数、全书估算页数和重新排版状态
- [ ] `EPUB-207` 使用章节 ID + DOM/字符锚点保存稳定阅读位置，不依赖动态页码
- [ ] `EPUB-208` JavaScript 选区桥接：目标句、上下文、屏幕锚点和稳定位置
- [ ] `EPUB-209` 复用现有侧边栏/弹窗解句及学习记录保存
- [ ] `EPUB-210` 目录面板和章节跳转
- [ ] `EPUB-211` 字号、行距、页边距、明暗主题及设置持久化
