...
- [ ] `AI-501` 扩展领域模型：句子类型、句型、成分、从句关系和整句理解路径
- [ ] `AI-502` 每个句法成分携带原文片段、语法角色、修饰对象和中文说明
- [ ] `AI-503` 校验句法原文片段必须来自目标句，防止上下文串线和虚构
- [ ] `AI-504` 保留“快速解句”，增加按需触发的“深入解析”模式
- [ ] `AI-505` 为深入解析设计独立短 Prompt、JSON Schema、解析与一次修复
- [ ] `AI-506` 评测 1.5B 深入解析的结构正确率、JSON 成功率和延迟
- [ ] `AI-507` 对照 3B 模型；若 1.5B 高频误导，则将 3B 设为深度解析推荐模型
- [ ] `AI-508` 解句面板按“翻译→句型→主干→成分→从句→语法”折叠展示
