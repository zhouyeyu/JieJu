...
- [ ] `AI-211` JSON Schema 字段携带目标语言，修复 `translation` 输出英文而非中文
      （字段名本身对 0.5B/1.5B 构成词汇诱导，需连同字段描述一起处理）
- [ ] `AI-212` 校验拦截 `translation` 与 `targetText` 完全相同
...
- [ ] `AI-411` 0.5B 与 1.5B 双模型批量对照，决定默认模型

> `AI-409`～`AI-411` 必须在 `AI-211`/`AI-212` 修完之后再跑。
> 当前状态下 `translation` 一律输出英文，批量评测会得到废数据。
