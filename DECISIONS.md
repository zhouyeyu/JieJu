...
## 2026-09-15：Windows PDF 使用本地 PDF.js 提供受控文字层

WebView2 内置 PDF 查看器可以显示本地文件，但应用无法可靠读取其扩展页面中的选区、上下文和
当前页。Windows 改为随包携带 Apache-2.0 的 PDF.js 5.6.205，在独立受限来源上按需提供当前文件，
并由 JieJu 自有页面生成画布和可选择文字层。文档请求只匹配当前已打开 PDF，不开放所在目录；
Reader CSP 禁止外网连接。首版关闭分段下载，以保证本地响应行为确定，后续针对接近 250 MB 上限
的文件增加 Range 响应和按需传输。

PDF 选区沿用 `selectionChanged` v1 消息，locator 使用共享 v5 已发布的零基 `pageIndex`、正文偏移
和选区 SHA-256。复杂竖排或注音 PDF 的抽取顺序可能与视觉顺序不同；找不到目标偏移时保留用户
实际选区并把偏移置空，不能猜造上下文位置。
