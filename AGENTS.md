...
- macOS 与 Windows 开发均使用独立任务分支，`main` 只接收经过审查的 PR，不直接提交或推送 main。
- 分支按任务命名，例如 `fix/macos-epub-selection`、`feature/windows-shell`；每个可验证里程碑创建
  一个面向 `main` 的 PR，说明影响平台、共享字段变化与实际测试结果。不要把不同任务累积为长期大 PR。
- 新任务从最新 `origin/main` 创建分支；已发布分支同步主线时保留历史，不强推或改写他人的提交。
- macOS 任务主要修改 `JieJu/`、`JieJuTests/` 和 Xcode 工程；Windows 任务主要修改 `Apps/Windows/`。
  `Shared/`、语言包和根文档属于共同区域；跨平台接口变更先单独提出 PR，明确迁移与两端验收后再集成。
- 用户已授权按里程碑推送任务分支和创建 PR；合并 PR 仍需明确授权。定期指任务阶段完成时，未设置定时自动任务。
