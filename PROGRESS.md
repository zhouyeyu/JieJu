...
- **EPUB 已升级为真正的书页阅读**（`EPUB-205B`/`206A`/`207A`/`214A`）：不再直接依赖原书
  `body` 做多栏，而是把章节正文装入隔离的固定视口容器，按窗口宽度逐页位移；原书的宽高、边距
  和 overflow 样式不会再让长章节退化为一页。底部显示“章节 + 本章页码”和进度条，图片、字体、
  窗口尺寸变化后自动重新分页；阅读位置现保存到章节和章内页，重启后可回到上次书页。
