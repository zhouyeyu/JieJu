...
public static partial class ExplanationValidation
{
    ...
    public static Explanation Parse(string raw, ExplanationRequest request)
    {
        ...
        if (!Contains(request.TargetText, result.SentenceCore) || result.GrammarPoints.Any(x => !Contains(request.TargetText, x.Text)) || result.KeyPhrases.Any(x => !Contains(request.TargetText, x.Text)))
            throw new JsonException("模型解释引用了选区之外的文字。");
        return result;
    }
